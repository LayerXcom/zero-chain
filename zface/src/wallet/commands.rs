...
use super::{DirOperations, KeystoreDirectory, WalletDirectory};
...
/// Create a new wallet
pub fn new_wallet<R: Rng>(term: &mut Term, root_dir: PathBuf, rng: &mut R) -> Result<()> {
    ...
    let mut keyfile_master = KeyFile::create_master(
        MASTER_ACCOUNTNAME,
        VERSION,
        &password[..],
        ITERS,
...
pub fn show_list(term: &mut Term, root_dir: PathBuf) -> Result<()> {
    let (wallet_dir, keystore_dir) = wallet_keystore_dirs(&root_dir)?;
...
pub fn new_keyfile<R: Rng>(term: &mut Term, root_dir: PathBuf, rng: &mut R) -> Result<()> {
    let (wallet_dir, keystore_dir) = wallet_keystore_dirs(&root_dir)?;
...
pub fn recover<R: Rng>(term: &mut Term, root_dir: PathBuf, rng: &mut R) -> Result<()> {
    // 1. configure wallet directory
    let (wallet_dir, keystore_dir) = wallet_keystore_dirs(&root_dir)?;
...
pub fn load_dec_key(term: &mut Term, root_dir: PathBuf) -> Result<DecryptionKey<Bls12>> {
    let (wallet_dir, keystore_dir) = wallet_keystore_dirs(&root_dir)?;
...
pub fn change_default_account(root_dir: PathBuf, account_name: &str) -> Result<()> {
    ...
    let mut updated_index_file =
        index_file_u.set_default_index(*index, keyfile_name.as_str(), account_name);
