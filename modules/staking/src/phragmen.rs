...
{
    let into_currency = |b: BalanceOf<T>| {
        <T::CurrencyToVote as Convert<BalanceOf<T>, u64>>::convert(b) as ExtendedBalance
    };
    let into_votes = |b: ExtendedBalance| {
            ...
            for e in &mut n.edges {
                ...
                if let Some(c) = elected_candidates.iter_mut().find(|c| c.who == e.who) {
                    ...
                    if c.who != n.who {
                        ...
                        c.exposure.total =
                            c.exposure.total.saturating_add(into_votes(e.backing_stake));
                        c.exposure.others.push(IndividualExposure {
                            who: n.who.clone(),
                            value: into_votes(e.backing_stake),
                        });
                    }
                }
            }
