...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn confidential_transfer(
            ...
            // Get encrypted fee with the option type
            let enc_fee = match enc_tx_fee.into_ciphertext() {
                Some(f) => f,
                None => return Err("Invalid enc_tx_fee"),
            };

            // Charge transaction fee to sender
            <EncryptedBalance<T>>::mutate(address_sender, |balance| {
                 let charged_balance = balance.clone().map(
                     |_| Ciphertext::from_ciphertext(&bal_sender.sub_no_params(&enc_fee)));
